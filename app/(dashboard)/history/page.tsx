...
import ManualPlanTabs from "@/components/history/ManualPlanTabs";
...
export default async function HistoryPage() {
  ...
  const completedPlans = (manualPlans ?? []).filter((p) => isCompletedPlan(p));
  const isEmpty = historyItems.length === 0 && (manualPlans ?? []).length === 0;
      ...
      {isEmpty ? (
        ...
        <div className="flex flex-col gap-10">
          {(manualPlans ?? []).length > 0 && (
            <section>
              <h2 className="font-serif font-bold text-lg text-ink mb-3">作成したプラン</h2>
              <ManualPlanTabs inProgress={inProgressPlans} completed={completedPlans} />
            </section>
          )}

          {historyItems.length > 0 && (
            <section>
              <h2 className="font-serif font-bold text-lg text-ink mb-3">保存した計算記録</h2>
